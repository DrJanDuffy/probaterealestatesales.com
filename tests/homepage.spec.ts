...
test.describe('Nevada Probate Real Estate - Homepage Tests', () => {
  ...
  test('should have probate-specific trust signals', async ({ page }) => {
    ...
    await expect(page.locator('text=/Nevada.*license|NV.*license|licensed.*Nevada/i')).toBeVisible();
  });
  ...
  test('should have comprehensive Las Vegas area coverage', async ({ page }) => {
    ...
    const locationLinks = await page.locator('a[href*="location"], a[href*="summerlin"], a[href*="henderson"]').count();
  ...
  test.afterEach(async ({ page }, testInfo) => {
    ...
    fs.appendFileSync('performance-log.json', 
      JSON.stringify({
        test: testInfo.title,
